    ...
    for i in root:  # 遍历一级节点
        ...
        if i.tag == 'object':
            for j in i:
                if j.tag == 'name':
                    cls_name = j.text
                cat_id = voc_clses.index(cls_name) + 1   #0位置留给background
                if j.tag == 'bndbox':
                    bbox = []
                    xmin = 0
                    ymin = 0
...
##############VOC格式标注文件XML路径#################
voc2007xmls = '.\Annotations'
##############VOC格式文件路径#######################
test_txt = './ImageSets/Main/trainval.txt'
##############导出coco style json格式文件路径########
