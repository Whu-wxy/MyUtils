...
def Count_SQuAD(file_path:str, use_spacy:bool=False):    
    ...
    with open(file_path, 'r') as dataset_file:
        ...
        # save2file('context_word', context_word)  #词数：0,1,2...
...
def paragraph_join(src_json, dst_text):
    """
    把一个大文章分出的小文章拼接起来——结果为：小文章顺序已经被打乱
    """
    with open(src_json, 'r', encoding='utf-8') as dataset_file:
        dataset_json = json.load(dataset_file)
        dataset = dataset_json['data']
        article = dataset[0]    #某一篇大文章
        paragraphs = article['paragraphs']
...
def get_min_or_max_count(file_name, isMin:bool=True):
    fileName = 'F:\\论文\\毕设\\SQuAD分析\\' + file_name
    number_list = []
    ...
    if isMin is False:   #求最大值的索引
        number_list.reverse()
    for idx, number in enumerate(number_list):
        if number != 0:
            if isMin is False:   #求最大值的索引
                return len(number_list)-idx-1
            else:
                return idx


def find_length1_id(file_path:str):
    nlp = spacy.load('en')
    ids = []
    with open(file_path, 'r') as dataset_file:
        dataset_json = json.load(dataset_file)
        ...
        for article in tqdm(dataset):
            ...
            for paragraph in paragraphs:
                ...
                for qa in qas:
                    question = qa['question']
                    if len(remove_spaces(nlp(question))) == 1:
                        ids.append(qa['id'])
    return ids
...
# print(get_min_or_max_count('train\\context_word.txt'))

ids = find_length1_id('F:\dl-data\datasets\SQuAD\\train-v1.1.json')
print(ids)